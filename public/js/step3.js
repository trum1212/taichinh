...
const FaceCaptureApp = {
    ...
    async handleImageVerification() {
        ...
        try {
            ...
            const ekycData = JSON.parse(localStorage.getItem('ekycData') || '{}');
            if (!ekycData.frontImageHash) {
                throw new Error('Không tìm thấy dữ liệu CCCD từ bước trước.');
            }
            const matchResult = await this.compareFace(ekycData.frontImageHash, faceHash);
